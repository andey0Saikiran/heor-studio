...
- **Overall survival.** MarketScan's only native death signal is `DSTATUS` —
  in-hospital only, masked from data year 2016. A curve built on it would silently
  become "time to in-hospital death before 2016, with every other death censored."
...
- **Bootstrap intervals** (need an RNG, break reproducible emission), **Gray's
  test**, **quantile parity between `PROC UNIVARIATE` and SQL** — each named with
  its reason in [`docs/ANALYSIS-BUILD-PLAN.md`](docs/ANALYSIS-BUILD-PLAN.md).
...
- **AIPW ≡ the g-formula.** With the score and the outcome model both saturated
  ...
  entirely different expressions — one over cells, one over subjects — check each
  other.
