...
async function main() {
  ...
  console.log("\n=== review queue (the gate, asked one question at a time) ===");
  for (const c of rq) console.log(`  ${c.status === "pass" ? "PASS" : "FAIL"}  ${c.name} — ${c.detail}`);
